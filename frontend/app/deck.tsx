...
export default function Deck() {
  ...
  const orderDetailsSlide = useRef(new Animated.Value(SCREEN_HEIGHT)).current
  ...
  // Hint becomes more visible as user drags downward (position.y is positive when moving down)
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: (_evt, { dx, vx, dy, vy }) => {
      ...
      const isSwipeDown = dy > V_DISTANCE || vy > V_VELOCITY
      if (isSwipeUp) showOrderDetailsAnimation()
      else if (isSwipeDown) onDecision('cart')
  ...
  const showOrderDetailsAnimation = () => {
    if (isAnimating || currentIndex >= items.length) return
    setShowOrderDetails(true)
    
    // Animate the card sliding up and out of view
    Animated.parallel([
      ...
      Animated.timing(orderDetailsSlide, {
        toValue: 0,
        duration: 400,
        useNativeDriver: true,
      }),
  ...
  const hideOrderDetailsAnimation = () => {
    Animated.parallel([
      Animated.timing(position, {
        toValue: { x: 0, y: 0 },
        duration: 400,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.cardStack}>
        ...
        >
          ...
          >
            ...
            {/* Upward hint for Order Details */}
            <Animated.View pointerEvents="none" style={[styles.upHint, { opacity: upHintOpacity }]}>
              <Text style={styles.upHintArrow}>↑</Text>
              <Text style={styles.upHintText}>Order details</Text>
            </Animated.View>
            
            {/* Downward hint for Add to Cart */}
            <Animated.View pointerEvents="none" style={[styles.downHint, { opacity: downHintOpacity }]}>
              <Text style={styles.downHintArrow}>↓</Text>
              <Text style={styles.downHintText}>Add to cart</Text>
            </Animated.View>
      ...
      <View style={styles.instructions}>
        <Text style={styles.instructionText}>
          Swipe right to love • Swipe left to pass • Swipe up for details • Swipe down to add to cart
        </Text>
      ...
      {/* Order Details Sliding Animation */}
      {showOrderDetails && currentIndex < items.length && (
        <Animated.View 
          style={[
            styles.orderDetailsOverlay,
            {
              transform: [{ translateY: orderDetailsSlide }]
            }
          ]}
        >
          <View style={styles.orderDetailsContent}>
            <View style={styles.orderDetailsHeader}>
              ...
              <Pressable onPress={hideOrderDetailsAnimation}>
                <Text style={styles.orderDetailsClose}>✕</Text>
              </Pressable>
            </View>
            
            {items[currentIndex] && (
              <ScrollView showsVerticalScrollIndicator={false} style={styles.orderDetailsScroll}>
                <View style={styles.orderDetailsSection}>
                  <Text style={styles.orderDetailsBrand}>{items[currentIndex].brand}</Text>
                  <Text style={styles.orderDetailsProductTitle}>{items[currentIndex].title}</Text>
                  <Text style={styles.orderDetailsPrice}>{formatPrice(items[currentIndex].price)}</Text>
                </View>
                
                <View style={styles.orderDetailsSection}>
                  <Text style={styles.orderDetailsSectionTitle}>Available Sizes</Text>
                  <View style={styles.orderDetailsSizes}>
                    {(items[currentIndex].sizes || []).map((size, index) => (
                      <Pressable key={index} style={styles.orderDetailsSizeChip}>
                        <Text style={styles.orderDetailsSizeText}>{size}</Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
                ...
                <View style={styles.orderDetailsSection}>
                  <Text style={styles.orderDetailsSectionTitle}>Available Colors</Text>
                  <View style={styles.orderDetailsColors}>
                    {(items[currentIndex].colors || []).map((color, index) => (
                      <Pressable key={index} style={styles.orderDetailsColorChip}>
                        <Text style={styles.orderDetailsColorText}>{color}</Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
                ...
                <View style={styles.orderDetailsActions}>
                  <Pressable
                    onPress={() => {
                      hideOrderDetailsAnimation()
                      setTimeout(() => onDecision('cart'), 500)
                    }}
                    style={styles.orderDetailsAddToCart}
                  >
                    <Text style={styles.orderDetailsActionText}>Add to Cart</Text>
                  </Pressable>
                  <Pressable
                    onPress={() => {
                      hideOrderDetailsAnimation()
                      setTimeout(() => onDecision('like'), 500)
                    }}
                    style={styles.orderDetailsLike}
                  >
                    <Text style={styles.orderDetailsActionText}>♡ Like</Text>
                  </Pressable>
                </View>
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  // Downward hint styles (centered near top of overlay)
  downHint: {
    position: 'absolute',
    top: '36%',
    left: 0,
